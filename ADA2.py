...
def find_shortest_path_bellman_ford(graph, start, end):
    shortest_path = nx.bellman_ford_path(graph, source=start, target=end, weight='weight')
    return shortest_path

# example of finding the shortest path between two points using Bellman-Ford
start_point = "A"
end_point = "E"
shortest_path_bf = find_shortest_path_bellman_ford(road_network, start_point, end_point)
print(f"Shortest path from {start_point} to {end_point} using Bellman-Ford: {shortest_path_bf}")
...
# highlight Bellman-Ford shortest path on the map
path_coords_bf = [node_coordinates[node] for node in shortest_path_bf]
folium.PolyLine(locations=path_coords_bf, color='green', weight=5).add_to(jakarta_map)

# display the map
jakarta_map.save("jakarta_road_network_bellman_ford.html")  # save the map to an HTML file
jakarta_map
